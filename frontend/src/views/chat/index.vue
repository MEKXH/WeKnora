<template>
    <div class="chat">
        <div ref="scrollContainer" class="chat_scroll_box" @scroll="handleScroll">
            <div class="msg_list">
                <!-- ⚡ Bolt: Use unique ID for key to optimize rendering when prepending history -->
                <div v-for="(session, index) in messagesList" :key="session.id">
...
const sendMsg = async (value, modelId = '', mentionedItems = []) => {
    ...
    // 将@提及的知识库和文件信息存入用户消息
    messagesList.push({
        // ⚡ Bolt: Generate temp ID for stable list rendering
